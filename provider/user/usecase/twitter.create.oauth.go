...
func (c *CreateOauthTw) Perform(ctx context.Context, request entity.OUserTwitter, db provider.DB) *entity.ApplicationError {
	result, err := db.ExecContext(ctx, "create-oauth-tw", "INSERT INTO oauth_twitter (user_id,name,user_name,profile_image_url,access_token,access_token_secret,user_account_id,created_at) "+
		"VALUES (?, ?, ?, ?, ?, ?, ?, now());",
