...
from tweet_api.twitter import TwitterError
import tweet_api.api
...
def parse_arg_list(args, attr):
    ...
    if isinstance(args, (str, unicode)):
        out.append(args)
    elif isinstance(args, tweet_api.api.twitter.User):
        out.append(getattr(args, attr))
    elif isinstance(args, (list, tuple)):
        for item in args:
            if isinstance(item, (str, unicode)):
                out.append(item)
            elif isinstance(item, tweet_api.api.twitter.User):
