...
from tweet_api.twitter.ratelimit import RateLimit
from tweet_api.twitter.twitter_utils import (
...
from tweet_api.twitter.error import (
    TwitterError, PythonTwitterDeprecationWarning330
)
...
class Api(object):
    ...
    def GetStatuses(self,
        ...
        while offset < len(status_ids):
            parameters['id'] = ','.join([str(enf_type(
                'status_id', int, status_id)) for status_id in status_ids[offset:offset + 100]])

            resp = self._RequestUrl(url, 'GET', data=parameters)
            data = self._ParseAndCheckTwitter(resp.content.decode('utf-8'))
            if map:
                result.update({int(key): (Status.NewFromJsonDict(
                    value) if value else None) for key, value in data['id'].items()})
            else:
                result += [Status.NewFromJsonDict(dataitem)
    ...
    def _TweetTextWrap(self,
        ...
        if len(words) == 1 and not is_url(words[0]):
            if len(words[0]) > CHARACTER_LIMIT:
                raise TwitterError(
                    "Unable to split status into tweetable parts. Word was: {0}/{1}".format(len(words[0]), char_lim))
            else:
        ...
        for word in words:
            if len(word) > char_lim:
                raise TwitterError(
                    "Unable to split status into tweetable parts. Word was: {0}/{1}".format(len(word), char_lim))
            new_len = line_length

            if is_url(word):
                new_len = line_length + \
                    self._config['short_url_length_https'] + 1
            else:
                new_len += len(word) + 1
    ...
    def PostUpdates(self,
        ...
        for tweet in tweets[0:-1]:
            results.append(self.PostUpdate(
                status=tweet + continuation, **kwargs))
            ...
            if url and self.sleep_on_rate_limit:
                ...
                if limit.remaining == 0:
                    try:
                        ...
                        logger.debug(
                            'Rate limited requesting [%s], sleeping for [%s]', url, stime)
                        time.sleep(stime)
                    except ValueError:
                        pass
        ...
        if verb == 'POST':
            if data:
                if 'media_ids' in data:
                    url = self._BuildUrl(url, extra_params={
                                         'media_ids': data['media_ids']})
                    resp = self._session.post(
                        url, data=data, auth=self.__auth, timeout=self._timeout, proxies=self.proxies)
                elif 'media' in data:
                    resp = self._session.post(
                        url, files=data, auth=self.__auth, timeout=self._timeout, proxies=self.proxies)
                else:
                    resp = self._session.post(
                        url, data=data, auth=self.__auth, timeout=self._timeout, proxies=self.proxies)
            elif json:
                resp = self._session.post(
                    url, json=json, auth=self.__auth, timeout=self._timeout, proxies=self.proxies)
            else:
            ...
            resp = self._session.get(
                url, auth=self.__auth, timeout=self._timeout, proxies=self.proxies)
